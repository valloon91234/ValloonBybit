...
namespace Valloon.Trading.Backtest
{
    static class SMA2
    {
        ...
        public static void Run()
        {
            ...
            Loader.Load(SYMBOL, 1, new DateTime(2022, 5, 8, 0, 0, 0, DateTimeKind.Utc)); return;
        ...
        static float Benchmark()
        {
            ...
            var list = Loader.LoadBinListFrom1m(interval, list1m);
            ...
            Logger logger = new Logger($"{DateTime.Now:yyyy-MM-dd  HH.mm.ss}    SMA2    bin = {interval}    fee = {makerFee} - {takerFee}    {startTime:yyyy-MM-dd} ~ {endTime:yyyy-MM-dd} ({totalDays:N0}) days");
            logger.WriteLine("\n" + logger.LogFilename + "\n");
            ...
            //int smaLength1 = 144;
            {
                ...
                //int smaLength2 = 72;
                {
                    ...
                    for (float stopX = 0.01f; stopX <= 0.05f; stopX += .005f)
                    //float StopX = .012f;
                    {
                        int tryCount = 0;
                        ...
                        {
                            var sma1 = smaList1[i].Sma.Value;
                            var sma2 = smaList2[i].Sma.Value;
                            var lastSma1 = smaList1[i - 1].Sma.Value;
                            var lastSma2 = smaList2[i - 1].Sma.Value;
                            if (position == 0)
                            {
                                if (lastSma1 >= lastSma2 && sma1 < sma2)
                                {
                                    position = 1;
                                    positionEntryPrice = list[i].Close;
                                }
                                else if (lastSma1 <= lastSma2 && sma1 > sma2)
                                {
                                    position = -1;
                                    positionEntryPrice = list[i].Close;
                                }
                            ...
                            {
                                if (list[i].Low > positionEntryPrice * (1 - stopX) || lastSma1 <= lastSma2 && sma1 > sma2)
                                {
                                    tryCount++;
                                    int close = (int)Math.Ceiling(positionEntryPrice * (1 - stopX));
                                    if (close < list[i].Low) close = list[i].Close;
                                    ...
                                    position = 0;
                                    if (lastSma1 <= lastSma2 && sma1 > sma2) i--;
                                }
                            }
                            else if (position == -1)
                            {
                                if (list[i].High > positionEntryPrice * (1 + stopX) || lastSma1 >= lastSma2 && sma1 < sma2)
                                {
                                    tryCount++;
                                    int close = (int)Math.Ceiling(positionEntryPrice * (1 + stopX));
                                    if (close > list[i].High) close = list[i].Close;
                        ...
                        if (finalPercent > 1f && succeedCount > 0)
                        {
                            ...
                            logger.WriteLine($"sma = {smaLength1} / {smaLength2} \t stop = {stopX:F4} \t count = {tryCount} / {succeedCount} / {failedCount} / {successRate:F2} \t % = {finalPercent:F4}");
                        }
                        else
                        {
                            logger.WriteLine($"sma = {smaLength1} / {smaLength2} \t stop = {stopX:F4} \t count = {tryCount} / {succeedCount} / {failedCount} / {successRate:F2} \t % = {finalPercent:F4}", ConsoleColor.DarkGray, false);
                        }
                    }
                }
            }
