            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    if (positionQty == 0)
                    {
                        ...
                        if (walletBalance < param.InvestAmount)
                        {
                            cancelAllBotOrders();
                        ...
                        if (startPrice == 0 || botUpperOrderList.Count > 0 || botLowerOrderList.Count == 0 || 1 - startPrice / lastPrice > param.ResetX)
                        {
                            cancelAllBotOrders();
                            decimal qty = decimal.Round(param.InvestAmount * position.Leverage.Value * (1 - (0.0006m * 2)) / lastPrice / param.OrderCount, param.QtyDecimals);
                            ...
                            //for (int i = 0; i < param.OrderCount; i++)
                            //{
                            //var price = decimal.Round(lastPrice * (1 - param.OrderStep * i - param.OrderPinned), param.PriceDecimals);
                            //{
                            //    var resultOrder = apiHelper.NewOrder(new OrderRes
                            //    {
                            //        Side = "Buy",
                            ...
                            //}
                            //{
                            //    var closePrice = decimal.Round(price + lastPrice * param.OrderStep, param.PriceDecimals);
                            //    var resultOrder = apiHelper.NewStopOrder(new ConditionalRes
                            //    {
                            //        Side = "Sell",
                            //        Symbol = symbol,
                            //        OrderType = "Limit",
                            ...
                            //        PositionIdx = 0
                            //    });
                            //    logger.WriteLine($"        [{BybitLinearApiHelper.ServerTime:HH:mm:ss fff}]  New Close Order:  price = {closePrice}    qty = {qty}");
                            //    logger.WriteFile("--- " + JObject.FromObject(resultOrder).ToString(Formatting.None));
                            //}
                            //}
                    ...
                    else
                    {
                        ...
                        decimal getLastFilledPrice()
                        {
                            Thread.Sleep(param.Interval * 1000);
                            var filledOrders = apiHelper.GetFullyFilledOrders(symbol, 1);
                            return filledOrders[0].Price.Value;
                        }
                        if (closeQtySum != position.Size.Value)
                        {
                            var price = decimal.Round(getLastFilledPrice() + startPrice * param.OrderStep, param.PriceDecimals);
                        ...
                        if (botLowerOrderList.Count == 0 && lastPositionQty != null && lastPositionQty.Value != positionQty)
                        {
                            var price = decimal.Round(getLastFilledPrice() - startPrice * param.OrderStep, param.PriceDecimals);
                            decimal qty = decimal.Round(param.InvestAmount * position.Leverage.Value * (1 - (0.0006m * 2)) / lastPrice / param.OrderCount, param.QtyDecimals);
                            var resultOrder = apiHelper.NewOrder(new OrderRes
